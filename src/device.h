...
public:
    ...
    struct Request {
        ...
        Request(RequestType type, const std::string& request, uint8_t index)
            : type(type), request(request), index(index)
        {
        }
...
private:
    ...
    //! Put requests into the request queue to read for all outputs the mapped inputs.
    void initialize();
    ...
    //! Put a request into the request queue or directly execute it if there is no request in progress.
    ...
    //! Protect the request queue from concurrent access by the io service and zeromq.
    ...
    //! The request which is currently sent to the device or whose response is being read and processed.
...
public:
     ...
     * @param output 1-based index of the output [1 <= output <= number_of_outputs]
     */
